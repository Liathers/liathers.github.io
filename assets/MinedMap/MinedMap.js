...
function signIcon(material, kind) {
	function createSignIcon(material, kind) {
		...
		return L.icon({
			iconUrl: `/assets/MinedMap/images/icon/${material}_${kind}.png`,
			iconSize,
			popupAnchor,
			shadowUrl: `/assets/MinedMap/images/icon/shadow_${kind}.png`,
...
const MinedMapLayer = L.TileLayer.extend({
	...
	getTileUrl: function (coords) {
		...
		return '/assets/MinedMap/data/'+getScriptData()+'/'+this.layer+'/'+z+'/r.'+coords.x+'.'+coords.y+'.png';
	},
});
...
function createSign(sign, back) {
	...
	container.style.backgroundImage = `url(/assets/MinedMap/images/bg/${sign.material}_${sign.kind}.png)`;
...
async function loadSigns(signLayer) {
	const response = await fetch('/assets/MinedMap/data/'+getScriptData()+'/entities.json', {cache: 'no-store'});
...
window.createMap = function () {
	(async function () {
		const response = await fetch('/assets/MinedMap/data/'+getScriptData()+'/info.json', {cache: 'no-store'});
		const res = await response.json();
